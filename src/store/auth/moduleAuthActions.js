            ...
            jwt.login(payload.userDetails.username, payload.userDetails.password)
                .then(response => {
                    ...
                    if (response.data) {
                        ...
                        let payloadJWT = JsonWebToken.decode(response.data.access)
                        //Nhan ham change role
                        payloadJWT.aclChangeRole = payload.aclChangeRole
                        // Set accessToken
                        localStorage.setItem("accessToken", response.data.access)
                        localStorage.setItem("refreshToken", response.data.refresh)

                        // Update user details
                        commit('UPDATE_USER_INFO', payloadJWT, { root: true })
                        // Set bearer token in axios
                        commit("SET_BEARER", response.data.access)

                        //Dang ky tag userid de push thong bao
                        //Them tags
                        window.OneSignal.push(function () {
                            window.OneSignal.sendTags({
                                "userid": payloadJWT.user_id,
                                "username": payloadJWT.username,
                                "is_superadmin": payloadJWT.is_superuser,
                                "is_admin": payloadJWT.is_admin,
                                "is_lead": payloadJWT.is_lead
                            }, function (tagsSent) {
                        ...
                        resolve(response)
                    } else reject({ message: "Loi! Wrong Email or Password" })
                    ...
                    //     resolve(response)
                    // } else {
                    //     reject({ message: "Loi! Wrong Email or Password" })
                    // }
