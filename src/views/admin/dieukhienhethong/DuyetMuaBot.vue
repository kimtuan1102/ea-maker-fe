...
<div id="data-list-list-view" class="data-list-container">
            ...
            <tbody>
                <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data ">
                    ...
                    <vs-td class="whitespace-no-wrap">
                        ...
                        <feather-icon title="Từ chối" icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.stop="rejectItem(tr.id,'reject')" />
                    </vs-td>
    ...
    <!-- Dialog duyet  -->
    <vs-prompt :is-valid="checkSeletedIdMt4" @cancel="valIdMt4=''" @accept="acceptDialog" @close="close" :active.sync="activePrompt" accept-text="Đồng ý" cancel-text="Đóng">
        <div class="con-exemple-prompt">
            Chọn từ danh sách
            <vs-select :input-changed="changeIdMt4($event)" :danger="valIdMt4==null" danger-text="Chưa chọn id mt4 cha" autocomplete class="w-full mb-5 mt-3" label="" v-model="selectMt4">
                <vs-select-item :key="index" :value="item.id" :text="item.name" v-for="(item,index) in lstAccountMp4Parent" />
            </vs-select>
            Id Mt4 đã chọn
            <vs-input disabled class="w-full mb-5 mt-3" placeholder="Id MT4 Cha" v-model="valIdMt4" />
        </div>
    </vs-prompt>
    <!-- End Dialog -->
</div>
...
export default {
    ...
    computed: {
        ...
        dataTransactions() {
            ...
            return this.$store.state.dataAdmin.dataAccountConfigs.reverse()
        },
        lstAccountMp4Parent() {
    ...
    methods: {
        ...
        acceptDialog() {
            this.approveItem(this.idSeleted, this.idUserSelected)
        },
        ...
        approveItem(id, idUser) {
            ...
            let payload = {
                id: id,
                body: {
                    "parent": this.valIdMt4
                }
            }
            this.$store.dispatch("dataAdmin/adminApproveBot", payload).then((response) => {
                    ...
                    // gui thong bao den supper admin
                    this.$sendNotify({
                        is_superadmin: true
                    }, {
        ...
        rejectItem(id) {
            this.$vs.loading()
            this.$store.dispatch("dataAdmin/adminRejectBot", id).then((response) => {
                    this.$vs.loading.close()
                    this.$vs.notify({
                        title: 'Thành công! ',
                        text: response.data,
